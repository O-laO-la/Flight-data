...
d3.csv("data/chord_top16.csv").then(function(data) {
    chordData = data;
...
function drawChord(period) {
    ...
    // 获取所有国家
    const countries = Array.from(new Set(filtered.flatMap(d => [d.origin_country, d.dest_country])));
    ...
    // 画布设置
    const width = 700, height = 750, innerRadius = 270, outerRadius = 290;
            ...
            tooltip
                ...
                .html(
                    `<strong>起点：</strong>${countries[d.source.index]}<br/>
                     ...
                     <strong>航班数：</strong>${matrix[d.source.index][d.target.index]}`
                )
