// 使用嵌入的数据初始化堆叠面积图
(function () {
  const data = embeddedStackedAreaData;
  // 数据预处理
  data.forEach((d) => {
  ...
  // 按国家分组
  const countries = Array.from(new Set(data.map((d) => d.country))).filter(
    (d) => d
  );
  // 构造宽表
  ...
  const stackedData = Array.from(dataByMonth.values()).sort(
    (a, b) => a.month - b.month
  );
      ...
      .join(
        (enter) =>
          enter
            ...
            .attr("opacity", 0.85)
            .attr("d", area),
        (update) => update.attr("fill", (d) => color(d.key)).attr("d", area),
        (exit) => exit.remove()
      )
        ...
        tooltip
          ...
          .html(
            `<strong>国家：</strong>${d.key}<br/>
                        <strong>月份：</strong>${d3.timeFormat("%Y-%m")(
                          datum.data.month
                        )}<br/>
  ...
  function zoomed(event) {
    ...
    // 更新面积
    chartArea.selectAll(".layer").attr(
      "d",
      d3
        .area()
